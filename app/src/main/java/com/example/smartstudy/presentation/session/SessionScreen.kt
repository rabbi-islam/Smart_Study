...
import com.example.smartstudy.util.Constant.ACTION_SERVICE_CANCEL
import com.example.smartstudy.util.Constant.ACTION_SERVICE_START
import com.example.smartstudy.util.Constant.ACTION_SERVICE_STOP
...
fun SessionScreenRoute(
    ...
    SessionScreen(
        ...
        onBackButtonIconClick = { navigator.navigateUp() },
...
private fun SessionScreen(
    ...
    timeService: StudySessionTimerService
) {
    val hours by timeService.hours
    val minutes by timeService.minutes
    val seconds by timeService.seconds
    val currentTimeState by timeService.currentTimerState
    ...
    LaunchedEffect(key1 = state.subjects) {
        val subjectId = timeService.subjectId.value
        onEvent(
    ...
    Scaffold(
        ...
        topBar = {
            SessionScreenTopBar(onBackButtonClick = onBackButtonIconClick)
        }
    ) { paddingValues ->
        LazyColumn(
            modifier = Modifier
            ...
            item {
                ButtonSection(
                    ...
                    startButtonClick = {
                        if (state.subjectId != null && state.relatedToSubject != null) {
                            ServiceHelper.triggerForegroundService(
                                context = context,
                                action = if (currentTimeState == TimerState.STARTED) ACTION_SERVICE_STOP
                                else ACTION_SERVICE_START
                            )
                            timeService.subjectId.value = state.subjectId
                        } else
                    ...
                    finishButtonClick = {
                        val duration = timeService.duration.toLong(DurationUnit.SECONDS)
                        if (duration >= 36) {
                            ServiceHelper.triggerForegroundService(
                                context = context,
                                action = ACTION_SERVICE_CANCEL
